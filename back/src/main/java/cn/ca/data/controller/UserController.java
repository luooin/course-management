...
import cn.ca.basics.redis.RedisTemplateHelper;
import cn.ca.basics.utils.*;
import cn.ca.basics.parameter.CommonConstant;
import cn.ca.basics.log.LogType;
import cn.ca.basics.baseVo.PageVo;
import cn.ca.basics.baseVo.Result;
import cn.ca.data.entity.*;
import cn.ca.data.service.*;
import cn.ca.data.utils.ZwzNullUtils;
import cn.ca.data.vo.PermissionDTO;
import cn.ca.data.vo.RoleDTO;
...
public class UserController {
    ...
    @SystemLog(about = "获取当前登录用户", type = LogType.DATA_CENTER,doType = "USER-02")      // 使用@SystemLog注解，自定义注解，用于记录系统日志，标注了关于“获取当前登录用户”的操作信息
    @RequestMapping(value = "/info", method = RequestMethod.GET)      // 使用@RequestMapping注解，定义了一个处理GET请求的/info路径
    @ApiOperation(value = "获取当前登录用户")      // 使用@ApiOperation注解，这是Swagger的注解，用于API文档生成，描述了该API的功能为“获取当前登录用户”
    public Result<User> getUserInfo(){
        User u = securityUtil.getCurrUser();      // 使用securityUtil工具类获取当前登录的用户信息
        entityManager.clear();      // 清除EntityManager中的持久化上下文，通常是为了避免缓存中的数据干扰到后续操作
        u.setPassword(null);      // 清除用户的密码信息，保证返回的数据不包含敏感信息
        return new ResultUtil<User>().setData(u);      // 创建一个Result对象，并设置数据为用户信息u，然后返回
    }
    // 注册用户接口    使用POST请求，映射到/regist路径
    @RequestMapping(value = "/regist", method = RequestMethod.POST)
    @ApiOperation(value = "注册用户")      // 使用Swagger的注解，描述该API的功能为“注册用户”
    public Result<Object> regist(@Valid User u){
        u.setEmail(u.getMobile() + "@qq.com");      // 设置用户的邮箱地址为其手机号加@qq.com后缀
        QueryWrapper<User> userQw = new QueryWrapper<>();      // 创建查询条件，检查用户名或手机号是否已存在
        userQw.and(wrapper -> wrapper.eq("username", u.getUsername()).or().eq("mobile",u.getMobile()));
        if(iUserService.count(userQw) > 0L) {      // 如果数据库中已存在相同用户名或手机号的用户，返回错误信息，提示登录账号/手机号重复
            return ResultUtil.error("登录账号/手机号重复");
        }
        String encryptPass = new BCryptPasswordEncoder().encode(u.getPassword());      // 使用BCrypt密码编码器对用户输入的密码进行加密
        u.setPassword(encryptPass).setType(0);       // 设置用户密码为加密后的密码，并设置用户类型为0（表示普通用户）
        iUserService.saveOrUpdate(u);      // 保存或更新用户信息
        QueryWrapper<Role> roleQw = new QueryWrapper<>();      // 查询默认角色
        roleQw.eq("default_role",true);
        List<Role> roleList = iRoleService.list(roleQw);      // 获取默认角色列表
        if(roleList.size() > 0){      // 如果存在默认角色，为新注册的用户分配默认角色
    ...
    // 解锁验证密码接口      使用POST请求，映射到/unlock路径      使用自定义的@SystemLog注解，记录系统日志，标注了关于“解锁验证密码”的操作信息
    @SystemLog(about = "解锁验证密码", type = LogType.DATA_CENTER,doType = "USER-03")
    @RequestMapping(value = "/unlock", method = RequestMethod.POST)
    @ApiOperation(value = "解锁验证密码")      // 使用Swagger的注解，描述该API的功能为“解锁验证密码”
    public Result<Object> unLock(@RequestParam String password){
        User u = securityUtil.getCurrUser();      // 获取当前登录用户信息
        if(!new BCryptPasswordEncoder().matches(password, u.getPassword())){      // 使用BCrypt密码编码器验证用户输入的密码是否正确
